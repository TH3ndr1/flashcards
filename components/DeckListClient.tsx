...
export function DeckListClient({ initialData = [] }: DeckListClientProps) {
  ...
  // const isLoading = settingsLoading; // Reverted this line, initialData implies data is ready for main structure
  ...
  // Reverted the settingsLoading check that returned a full-page spinner
  // if (settingsLoading) { 
  //   return (
  ...
  // showDeckProgress will use the setting if available, otherwise default to true.
  // This might cause a quick flash if settings load and change this value after initial render.
  const showDeckProgress = settings?.showDeckProgress ?? true;

  // If initialData is not yet available (though it should be from server component),
  // or if settings are loading and we want to wait for them for the main content rendering:
  // For now, we assume initialData means the primary content can be rendered.
  // The settingsLoading only affects the conditional rendering of progress bars within the map.
  if (settingsLoading && decks.length === 0) { // Show spinner only if truly no initial data AND settings are loading
    return (
      <div className="flex justify-center items-center h-64">
        <div className="h-12 w-12 rounded-full border-2 border-primary animate-spin border-b-transparent" />
      </div>
