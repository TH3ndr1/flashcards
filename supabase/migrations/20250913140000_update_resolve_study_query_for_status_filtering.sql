-- Update resolve_study_query function to filter by status
-- This ensures that study sets and smart playlists only include active cards from active decks
-- PRESERVES ALL EXISTING BUSINESS LOGIC - only adds status filtering

DROP FUNCTION IF EXISTS public.resolve_study_query(uuid, jsonb, uuid, real);

CREATE OR REPLACE FUNCTION public.resolve_study_query(
    p_user_id uuid,
    p_input_criteria jsonb DEFAULT NULL,
    p_study_set_id uuid DEFAULT NULL,
    p_random_seed REAL DEFAULT NULL
)
RETURNS SETOF uuid
LANGUAGE plpgsql
STABLE
...
BEGIN
    ...
    -- *** ONLY NEW CODE: Add status filtering for both cards and decks ***
    ...
    IF v_contains_language IS NOT NULL AND char_length(v_contains_language) = 2 THEN
        ...
        -- *** ONLY NEW CODE: Add deck status filtering for the language join ***
    ...
    -- *** ONLY NEW CODE: Ensure deck status filtering when no deck joins exist ***
    -- This handles cases where no language filter is applied but we still need deck status filtering
    IF v_join_clauses !~ 'JOIN public.decks' THEN
    ...
    -- *** ONLY NEW CODE: Add deck status filtering for tag joins ***
    -- The tag filtering uses EXISTS subqueries, but we need to ensure those also filter by deck status
    IF array_length(v_include_tags, 1) > 0 THEN
        -- The tag filtering already uses EXISTS subqueries, we need to modify those to include deck status
        -- This is handled by updating the EXISTS subqueries to join with decks and filter by status
        -- We'll do this by replacing the last added tag filter clauses
        IF v_tag_logic = 'ALL' THEN
            -- Remove the last added clause and replace it with status-aware version
            v_where_clauses := v_where_clauses[1:array_length(v_where_clauses,1)-1];
            v_where_clauses := array_append(v_where_clauses, format(
                'EXISTS (SELECT 1 FROM public.deck_tags d_all_tags JOIN public.decks deck_all ON d_all_tags.deck_id = deck_all.id WHERE d_all_tags.deck_id = c.deck_id AND d_all_tags.tag_id = ANY(%L::uuid[]) AND deck_all.status = ''active'' GROUP BY d_all_tags.deck_id HAVING COUNT(DISTINCT d_all_tags.tag_id) = %s)',
                v_include_tags, array_length(v_include_tags,1) ));
        ELSE -- ANY
            -- Remove the last added clause and replace it with status-aware version  
            v_where_clauses := v_where_clauses[1:array_length(v_where_clauses,1)-1];
            v_where_clauses := array_append(v_where_clauses, format(
                'EXISTS (SELECT 1 FROM public.deck_tags d_any_tags JOIN public.decks deck_any ON d_any_tags.deck_id = deck_any.id WHERE d_any_tags.deck_id = c.deck_id AND d_any_tags.tag_id = ANY(%L::uuid[]) AND deck_any.status = ''active'')',
                v_include_tags ));
        END IF;
    END IF;
...
COMMENT ON FUNCTION public.resolve_study_query(uuid, jsonb, uuid, real)
IS 'Resolve study query criteria to card IDs, filtering only active cards from active decks.';

GRANT EXECUTE ON FUNCTION public.resolve_study_query(uuid, jsonb, uuid, real) TO authenticated;
