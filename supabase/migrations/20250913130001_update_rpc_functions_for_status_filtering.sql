...
-- Update the get_decks_with_complete_srs_counts function to filter by status
-- THIS VERSION USES THE EXACT BUSINESS LOGIC PROVIDED BY THE USER.
...
deck_base AS (
  ...
  WHERE
    d.user_id = p_user_id
    AND d.status = 'active' -- ONLY ADDED THIS LINE
),
srs_counts AS (
  SELECT
    c.deck_id,
    -- EXACT LOGIC FROM USER
  ...
  WHERE
    c.user_id = p_user_id
    AND c.status = 'active' -- ONLY ADDED THIS LINE
